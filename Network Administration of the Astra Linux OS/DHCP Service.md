...
DHCPACK — получив DHCPREQUEST и его идентификатор, сервер проверяет, свободен ли запрошенный адрес.
             Если да — отправляет DHCPACK и вносит запись в базу, иначе отправляет DHCPNACK. Получив DHCPNACK,
             клиент должен убедиться в уникальности IP средствами протокола ARP и зафиксировать суммарный срок аренды.
...
Сценарий
Развернуть DHCP-сервер с динамическим обновлением DNS для домена ``localnet.example.ru (подсеть 192.168.32.0/24)``. DNS-сервер (BIND9) уже настроен на том же хосте.
